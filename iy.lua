...
local Current_Connections, Hooked_Connections = {}, {}
local good_check, XVNP_LASTUPDATE, XVNP_UPDATEINTERVAL = 0, 0, 5
...
pcall(function()
    ...
    Lua_Set_Upvalue(OPENAC_FUNCTION, 1, function(...) task.wait(200) end)
    hookfunction(OPENAC_FUNCTION, function(...) return {} end)
end)
end

local XVNP_CONNECTIONSNIFFER = Game_RunService.RenderStepped:Connect(function()
...
if tick() - XVNP_LASTUPDATE >= XVNP_UPDATEINTERVAL then
    ...
    local conns = Lua_Fetch_Connections(Game_LogService_MessageOut)
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- phowg Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs[#CMDs + 1] = {NAME = 'Hydroxide / hydroxide', DESC = 'Loads and initializes the Hydroxide script.'}
...
addcmd('Hydroxide', {'hydroxide'}, function(args, speaker)
    notify("Loading", 'Initializing Hydroxide...')
    ...
    local function webImport(file)
        ...
        local scriptContent = game:HttpGetAsync(url)
        return loadstring(scriptContent, file .. '.lua')()
    end

    webImport("init")
    webImport("ui/main")
    
    notify("Success", 'Hydroxide loaded successfully!')
end)
...
task.spawn(function()
	...
	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify("Meow meow meow meow meow")
		end
